...
import { formatShortAddress } from "@/entities/user";
...
export function HomeHeader({ query, onQueryChange }: HomeHeaderProps) {
  ...
  const location = formatShortAddress(user?.address) || "주소를 등록해주세요";
    ...
    <header className="sticky top-0 z-20 bg-gradient-to-b from-brand to-brand-dark px-4 pb-3.5 pt-4 shadow-[0_4px_20px_rgba(31,181,176,0.18)]">
      <div className="flex items-center justify-between gap-2">
        <button
          ...
          aria-label="배달 위치"
        >
          <span className="line-clamp-1 text-[18px] font-bold tracking-tight">
            {location}
          </span>
          <svg
            width="20"
